...
// (e.g. store_pipeline.rpt_daily_sales).

import { Pool } from 'pg';

const schema = process.env.NEXT_PUBLIC_SUPABASE_SCHEMA || 'store_pipeline';
const table = process.env.NEXT_DB_TABLE || 'rpt_daily_sales';
...
export async function fetchPayments() {
  ...
  const query = `SELECT * FROM ${qualifiedPaymentsTable} ORDER BY sale_date LIMIT 50`;
