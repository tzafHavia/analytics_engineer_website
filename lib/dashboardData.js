...
export async function fetchOverviewDashboardData(rawFilters = {}) {
  ...
  const [
    ...
    // Daily performance for selected period (all rows in range, newest first)
    query(
      `SELECT sale_date::text, total_sales_amount, total_units_sold, ticket_count, avg_ticket_amount
       FROM store_pipeline.rpt_daily_sales
       WHERE sale_date BETWEEN $1 AND $2
...
// ─── Sales tab fetch ──────────────────────────────────────────────────────────
export async function fetchSalesDashboardData(rawFilters = {}) {
  const EMPTY = {
    currentPeriod: { start: null, end: null, days: 30 },
    previousPeriod: { start: null, end: null, days: 30 },
...
export async function fetchProductsDashboardData() {
  const EMPTY = {
