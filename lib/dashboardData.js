...
async function fetchWorkforceDashboardData_core(filters = {}) {
  ...
  try {
    if (!scoped) {
      ...
      const [summaryRows, dailyRows, boundsRows, staffingVsSales] = await Promise.all([
    ...
    const [shiftRows, salesRows, dailyRows, staffingVsSales] = await Promise.all([
    ...
    if (!shiftRows.length) {
      return { ...EMPTY, staffingVsSales, period: { start, end, label, scoped: true } };
    }
