...
async function fetchInventoryDashboardData_core() {
  ...
  try {
    const [riskRows, histRows, scatterRows, actionRows, deadRows, healthTrendRows] = await Promise.all([
      ...
      // Inventory health trend: full daily time-series (first inventory-domain series)
      query(
    ...
    return { snapshotDate, kpis: { totalItems, outOfStockCount, stockoutRiskCount, overstockCount, deadStockCount, avgDaysOfCover }, inventoryDistribution, coverHistogram, scatterData, reorderItems, overstockItems, deadStockItems, healthTrend, healthTrendSummary };
  } catch (e) {
...
async function fetchWorkforceDashboardData_core(filters = {}) {
  ...
  try {
    ...
    const [shiftRows, salesRows, dailyRows] = await Promise.all([
